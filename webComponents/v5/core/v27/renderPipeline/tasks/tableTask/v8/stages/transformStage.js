 ...
 * Contains guard: Only processes if the composite key exists in inTemplates.composite
...
export const transformSpecTreeThroughSteps = ({
    ...
    for (const step of stepsList) {
        ...
        if (typeof localBuilder !== "function") {
            if (localLogUnfoundKeys) {
                console.warn(
                    `[Composite Guard] Step ${stepIndex} ("${step.name}") has no matching builder function for "${step.builder}".`
                );
            }
            continue;
        }
