...
export const applyStepToSpecTree = ({ inSpecTree, inStep, inValue }) => {
...
export const createBaseTree = ({ inCompositeDef, inTemplates = specTemplatesDictionary } = {}) => {
    ...
    if (localCompositeDef?.composite && localTemplates?.composite?.[localCompositeDef.composite]?.template) {
        return JSON.parse(JSON.stringify(localTemplates.composite[localCompositeDef.composite].template));
    }
