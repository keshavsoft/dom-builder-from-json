import { applyBindings } from "./bindingsEngine.js";
...
export const universalBuilderMap = {
    tableHeaderCell: ({ inDataModel, inTemplates }) => {
        ...
        const columns = localDataModel?.tHead || localDataModel?.columns || (Array.isArray(localDataModel) ? localDataModel : []);
    ...
    cardItem: ({ inDataModel, inTemplates }) => {
        const localDataModel = inDataModel;
        const localTemplates = inTemplates;

        const cards = localDataModel?.cards || (Array.isArray(localDataModel) ? localDataModel : []);
        const composite = localTemplates?.composite?.cardItem;

        if (!composite?.template || !Array.isArray(cards)) {
            return [];
        }
