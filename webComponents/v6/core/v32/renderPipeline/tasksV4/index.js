...
const createGenericSpecTask = ({
    ...
    const localCollectionKey = inCollectionKey;
    const localDataModel = inDataModel;
    const localBuilderMap = inBuilderMap;

    return ({ inDataModels, inCollections } = {}) => {
        const localCollections = inCollections;
        const localDataModels = inDataModels;

        const resolvedDataModel = (localCollectionKey && localCollections?.[localCollectionKey])
            || localCollections?.[localKey]
            || localDataModels?.[localKey]
            || localDataModel;
...
const buildTasks = ({
    ...
    for (const [taskName, taskDef] of Object.entries(localTasksDefinitions)) {
        ...
        const taskFactory = ({ inDataModel, inCollections, inShowSearch } = {}) => {
            const localDataModel = inDataModel;
            const localCollections = inCollections;
            ...
            return createGenericSpecTask({
                ...
                inCollectionKey: taskDef.collection,
                inDataModel: localDataModel,
                inBuilderMap: localBuilderMap
            });
        };

        taskRegistry[taskName] = taskFactory;
        taskRegistry[dataModelKey] = taskFactory;
    }
...
export const createCardTask = tasks.cardTask;
